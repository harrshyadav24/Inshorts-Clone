      ...
      <Container>
        <Grid container>
          <Grid lg={5} md={5} sm={5} xs={12} item>
            <Image src={props.article.url} />
          </Grid>
          <RightContainer lg={7} md={7} sm={7} xs={12} item>
            <Title>{props.article.title}</Title>
            <Author>
              <Short>short</Short> by {props.article.author} / {new Date(props.article.timestamp).toDateString()}
            </Author>
            <Description>{props.article.description}</Description>
            <Publisher>
              read more at <a href={props.article.link} target='_blank'>{props.article.publisher}</a>
            </Publisher>
          </RightContainer>
        </Grid>
      </Container>
